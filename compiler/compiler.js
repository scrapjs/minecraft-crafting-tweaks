...
let generateModuleRecipes = (options)=>{
    ...
    Array.from(Object.entries(options.blocks)).forEach(([key, obj])=>{
        if ((disallowedData[usedMCVersion].indexOf(obj.mc_version) == -1 || !obj.mc_version) && (options.type != "block" ? obj[options.type] : obj[options.from]) && (options.single ? (options.type != "block" ? obj[options.type] : obj)["single"] : true)) {
            let criterias = {};
